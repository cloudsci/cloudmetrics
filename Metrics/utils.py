...
def periodic(field,con):
    ...
    clouds_in_the_north = []
    for ix in range(nx):
        if (cld_lbl[y0,ix] > 0 and cld_lbl[y1,ix] > 0 and cld_lbl[y0,ix] != cld_lbl[y1,ix] and cld_lbl[y0,ix]):
    ...
    clouds_in_the_east = []
    for iy in range(ny):
        if (cld_lbl[iy,x0] > 0 and cld_lbl[iy,x1] > 0 and cld_lbl[iy,x0] != cld_lbl[iy,x1]):
    ...
    for cloud in np.unique(cld_lbl):
        ...
        if cloud in clouds_to_move_north:
            ...
            shift_y = ny
            if clouds_in_the_north[clouds_to_move_north.index(cloud)] in clouds_to_move_east:
        ...
        if cloud in clouds_to_move_east:
            shift_x = nx
            if clouds_in_the_east[clouds_to_move_east.index(cloud)] in clouds_to_move_north:
                shift_y = ny 
