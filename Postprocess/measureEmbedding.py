...
def computeDistance(dfMetrics, dfOvlIn, ovlDist=256, nRand=3, seed=True, 
                    shuffle=True, return_iRand=False):
    ...
    dists[:,1:] = np.sqrt(np.sum((rand-anch[:,np.newaxis,:])**2,axis=2).astype('float32'))
...
def getDistribution(dfMetrics, dfOvlIn, ovlDist=256, nShuf=100, seed=False,
                    shuffle=False):
    ...
    dfOvl, dists = computeDistance(dfMetrics, dfOvlIn, ovlDist, nShuf, seed, 
                                   shuffle)
...
def analyseOverlap(ndDfMet, dfOvlIn,savePath):
    ...
    mnNear = np.mean(dfOvl['nearDist']); stNear = np.std(dfOvl['nearDist'])
    mnRand = np.mean(dfOvl['randDist']); stRand = np.std(dfOvl['randDist'])
    
    print('Mean distance/std for overlapping scenes: ', mnNear,stNear)
    print('Mean distance/std for random scenes:      ', mnRand,stRand)
    
    ax = sns.kdeplot(dfOvl['nearDist'],shade=True,label='Overlapping scenes',color='midnightblue')
    ax = sns.kdeplot(dfOvl['randDist'],shade=True,label='Random scenes',color='darkseagreen')
    ...
    simRat = 1. - dfOvl['nearDist']/dfOvl['randDist']
    mnRat  = np.mean(simRat); stRat = np.std(simRat)
    ...
    print('Fraction of overlap distribution <'+str(per)+' of random scenes:    ', nearFrac)
    ...
    ax1.set_xlim((-0.5,1)); ax1.set_ylim((0,2.25))
    ax1.plot([mnRat,mnRat],[0,2.25],color = 'midnightblue',linestyle='--',label=r'$\overline{S}$')
