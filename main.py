...
downloadDirs = [rootDir+'/Data/Download/DataAqua',
...
cfgM = {
        ...
        'layers'     : ['MYD06_L2___Cloud_Mask_1km',
...
worldviewClient.downloadMODISImgs(cfg['startDate'],cfg['endDate'],cfg['extent'],
                                  cfg['savePath'],satellite=cfg['satellite'])

#%% Request data from LAADS archive through MODAPS API
modapsClient = ModapsClient()

# Search for correct datasets in LAADS archive
fileIDs = modapsClient.searchForFiles(cfgM['product'], 
                                      cfg['startDate'], cfg['endDate'], 
                                      cfg['extent'][3], cfg['extent'][2], 
                                      cfg['extent'][1], cfg['extent'][0], 
                                      dayNightBoth=u'D',
                                      collection=cfgM['collection'])
# Make an order
orderIDs = modapsClient.orderFiles(cfgM['email'], fileIDs, 
                                   doMosaic=True, 
...
for i in range(len(orderIDs)):
    ...
    if orderStatus == 'Available':
        cmd  = 'wget -e robots=off -m -np -R .html,.tmp -nH --cut-dirs=3 \
                https://ladsweb.modaps.eosdis.nasa.gov/archive/orders/'+ \
                orderID+'/ --header "Authorization: Bearer '+cfgM['appKey']+ \
                '" -P '+cfg['savePath']
...
dfMetrics, data = analysis.loadMetrics(metricDirs[0], metricsPP,
                                       return_images=False)
pca = PCA();  xPca = pca.fit_transform(data)

dfMetrics1, data1 = analysis.loadMetrics(metricDirs[0], metricsPP,
                                         return_images=False,ext=sensLabels[0])
                                                  
pca1   = PCA(); xPca1 = pca1.fit_transform(data1)

dfMetrics2, data2 = analysis.loadMetrics(metricDirs[0], metricsPP,
                                         return_images=False,ext=sensLabels[1])
pca2   = PCA(); xPca2 = pca2.fit_transform(data2)

dfMetrics3, data3 = analysis.loadMetrics(metricDirs[0], metricsPP,
                                         return_images=False,ext=sensLabels[2])
pca3   = PCA(); xPca3 = pca3.fit_transform(data3)
